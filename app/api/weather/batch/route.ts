...
type Point = { key?: string; lat: number; lng: number; date?: string };
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Basic validation
    const valid = points
      .map((p) => ({ ...p, lat: Number(p.lat), lng: Number(p.lng), date: toISODate(p.date ?? undefined) }))
    ...
    const makeUrl = (lat: number, lng: number, date?: string) => {
    ...
    for (let i = 0; i < valid.length; i += concurrency) {
      ...
      const settled = await Promise.allSettled(
        batch.map(async (p) => {
          const url = makeUrl(p.lat, p.lng, p.date);
          ...
          const key = p.key ?? `${p.lat.toFixed(3)},${p.lng.toFixed(3)}@${p.date ?? 'na'}`;
